...
public interface ICartDetailRepository extends JpaRepository<CartDetail, Integer> {
    ...
    @Query(value = "SELECT sum(cd.quantity * p.price) as totalMoney from cart_detail cd join product p on cd.product_id = p.id join cart ca on cd.cart_id = ca.id where cd.flag_deleted = false and cd.flag_status = false and ca.customer_id = :idCustomer", nativeQuery = true)
